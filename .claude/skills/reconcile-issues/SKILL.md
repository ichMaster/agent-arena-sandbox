...
description: Review one version's already-generated spec/implementation/vXX.YY-issues.md against the REAL current implementation and correct any issue that has drifted (stale file names, changed signatures, evolved contracts, or work already done). Edits the issues file in place with a visible "Reconciled" change-mark. Corrects issues only - never implements code or changes the version.
---
...
`spec/implementation/vXX.YY-issues.md`, compare each issue's assumptions against the **actual current
...
```
/reconcile-issues <vXX.YY | path-to-issues-file>
```

- `/reconcile-issues v03.02` → reconciles `spec/implementation/v03.02-issues.md`
...
3. Read the prior versions' `spec/implementation/*-execution-report.md` and any
...
4. Read [roadmap.md](../../../spec/roadmap.md) §`vXX.YY` (the DoD) + `CLAUDE.md`.

### Step 1: Find the drift

For each issue, compare its **assumptions** against reality:
- Wrong or renamed **file/module** paths (e.g. `client/agent.py` vs the real `agent/agent.py`).
...
For each issue that drifted, edit its section in `vXX.YY-issues.md`:

1. **Fix the details** — the Description / What-needs-to-be-done / Acceptance criteria — so they match
   the real implementation. Keep the **ARENA id and the intent**; correct only what drifted.
2. **Add a visible change-mark** as a blockquote directly under the issue heading, e.g.:
   ...
   > the shipped `agent/agent.py` behind the `LLMClient` seam (Anthropic). Reason: matches the real
   > implementation.
...
Commit the corrected issues file (`docs: reconcile vXX.YY issues against the implementation`, with the
...
- **Every change is marked.** Preserve the ARENA id + intent; add a dated `⟳ Reconciled` blockquote for
