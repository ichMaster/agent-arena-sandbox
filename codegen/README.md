...
This folder is the **subject-independent half** of the repo. Everything outside it —
`server/`, `games/`, `agent/`, `web/`, `tests/` — is regenerable output produced by the SDLC
skills in `.claude/skills/`. This folder watches that happen and records it.

Nothing here imports from the generated application, and nothing here is deleted when the
application is reset. That is deliberate: the dashboard must start and serve with the whole
application tree absent, which is its normal state between runs.
...
Port **8420**, never 8000 — the generated application owns 8000 and both may run at once.
...
|---|---|---|
| **Run header** | How long, where are we now, when will it end | `Now` names the *deepest running node* (`v01.03 · ARENA-016 · validating`). **ETA is a range, never a point**, and is blank until at least one version has finished — an ETA computed from nothing is worse than no ETA. |
