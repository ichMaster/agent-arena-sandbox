...
def test_an_issue_that_validated_and_emitted_is_compliant(isolated_runs_dir: Path) -> None:
    run = _write(
        isolated_runs_dir,
        [
            _event("issue.start", scope=_issue_scope("ARENA-001")),
            _pytest_run(),
            _event("issue.validate.end", scope=_issue_scope("ARENA-001")),
            _event("issue.end", scope=_issue_scope("ARENA-001")),
        ],
    )
...
def test_an_issue_that_validated_without_emitting_is_caught(isolated_runs_dir: Path) -> None:
    ...
    assert report.unemitted_issues == ["ARENA-001"]
...
def test_each_issue_is_judged_separately(isolated_runs_dir: Path) -> None:
    run = _write(
        ...
        [
            ...
            _event("issue.validate.end", scope=_issue_scope("ARENA-001")),
            _event("issue.end", scope=_issue_scope("ARENA-001")),
            _event("issue.start", scope=_issue_scope("ARENA-002")),
            _pytest_run(),
            _event("issue.end", scope=_issue_scope("ARENA-002")),
        ],
    )
    report = reconcile.reconcile(run, git_shas=set())
    assert (report.validate_observed, report.validate_emitted) == (2, 1)
    assert report.unemitted_issues == ["ARENA-002"]
